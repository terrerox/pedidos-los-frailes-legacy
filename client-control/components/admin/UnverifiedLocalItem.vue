<template>
  <div class="flex mt-2 lg:flex-row flex-col mb-4 items-center">
    ...
    <a v-tooltip="'Escribir por Whatsapp'" target="_blank" :href="API_SEND_WHATSAPP_MESSAGE + local.phoneNumber" class="flex-no-shrink block w-full p-2 ml-4 mr-2 border-2 rounded bg-green-500 hover:bg-green-600">
      <img class="m-auto" :src="whatsappIcon">
    </a>
    <a v-tooltip="'Llamar por teléfono'" class="flex-no-shrink block w-full p-2 ml-4 mr-2 border-2 rounded bg-gray-500 cursor-pointer hover:bg-gray-600" @click="phoneTrigger(local.phoneNumber)">
      <img class="m-auto" :src="phoneIcon">
    </a>
    <a v-tooltip="'Verificar'" class="flex-no-shrink cursor-pointer block w-full p-2 ml-2 border-2 rounded bg-blue-700 hover:bg-blue-800" @click="verifyLocalButton(local.accountId)">
